...
public class VerifyPhone extends AppCompatActivity implements OnHttpCallCompleted {
    ...
    private void handleBokuResponse(HttpResponse httpResponse) {
        setProgressVisibility(false);
        BokuResponseStatus status = getBokuStatus(httpResponse);
        if (status == BokuResponseStatus.SUCCESS) {
            //phone verified
    ...
    private BokuResponseStatus getBokuStatus(HttpResponse httpResponse) {
        ...
        BokuResponseStatus status = BokuResponseStatus.fromValue(objStatus.toString());
        return status;
    }
